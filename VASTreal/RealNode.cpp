...
/*
 *  RealNode.cpp -- basic node entity used for simulations with bookkeeping capability
 *
 */
...
#include "RealNode.h"
    ...
    RealNode::RealNode (int node_no, MovementGenerator *move_model, const char *GWstr, SimPara &para, VASTPara_Net &netpara) //, bool as_relay)
    ...
    {                     
        ...
        _steps = 0;
        Position *pos = _move_model->getPos (_nodeindex, _steps);
...
//            std::cout << "Current timestamp: " << getVerse()->getTimestamp() << std::endl;
    ...
    void RealNode::recordStatPerSecond ()
    {
    ...
    */
    ...
    StatType &RealNode::getSendStat (bool interval_only)
    {
        return _world->getSendStat (interval_only);
    }

    StatType &RealNode::getRecvStat (bool interval_only)
    {
        return _world->getReceiveStat (interval_only);
    }

    length_t RealNode::getAOI ()
    {
        return _self.aoi.radius;
    }

    Position &RealNode::get_pos ()
    {
        return _self.aoi.center;
    }

    Node *RealNode::getSelf ()
    {
        return &_self;
    }

    VASTVerse *RealNode::getVerse() {
        return _world;
    }

    Vast::id_t RealNode::getID ()
    {
        return _self.id;
    }

    // get subscription ID
    Vast::id_t RealNode::getHostID ()
    {
    ...
    /*
    inline size_t RealNode::accumulated_send ()
    {
        //return _world->getSendSize ();
        return _world->getSendStat ().total;
    }

    inline size_t RealNode::accumulated_recv ()
    {        
    ...
    */
    ...
    long RealNode::min_aoi ()
    {
        return _min_aoi;
    }

    float RealNode::avg_aoi ()
    {
    ...
    */
	...
	// distance to a point
    bool 
    RealNode::in_view (RealNode *remote_node)
    {
