...
/*
 *  RealNode.h -- basic node entity used for simulations with bookkeeping capability
 *
 */
...
public:
    ...
    RealNode (int id, MovementGenerator *move_model, const char *GWstr, SimPara &para, VASTPara_Net &netpara); //, bool as_relay);
	...
	// distance to a point
    bool in_view (RealNode *remote_node);
    
    // returns the Node pointer if known, otherwise returns NULL
    // TODO:
    Node *knows (RealNode *node);
    ...
    std::unique_ptr<VASTLatencyStatLogEntry>   pLatencyStatlog = nullptr;
