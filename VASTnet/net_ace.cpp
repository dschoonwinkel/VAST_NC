...
namespace Vast {
    ...
    // constructor
    net_ace::net_ace (uint16_t port, const char* bindAddress)
        :_port_self (port), 
         _up_cond (NULL), 
         _down_cond (NULL), 
         _acceptor (NULL)
    ...
    connect (id_t target, unsigned int host, unsigned short port, bool is_secure)
    {
        ...
        {
            printf ("net_ace::connect () connection for [%lu] already exists\n", target);
            return true;
        }
    ...
    net_ace::switchID (id_t prevID, id_t newID)
    {
        ...
        {
            printf ("[%lu] net_ace::switchID () old ID not found or new ID already exists\n", _id);
        }
        else
        {
            printf ("[%lu] net_ace::switchID () replace [%lu] with [%lu]\n", _id, prevID, newID);
